...
export async function POST(req: Request) {
  try {
    ...
    const candidates = await findCorpusCandidates(word, 300);
...
${JSON.stringify(candidates.map((item) => item.sentence), null, 2)}`;
    ...
    const normalizedMeanings = (parsed.meanings ?? []).map((item) => ({
      meaning: ensureMeaningTag(item.meaning ?? ""),
      example: {
